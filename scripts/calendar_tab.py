...
import calendar
import pandas as pd
from .data_manager import load_expenses, MONTHS_FR, save_agenda_note, load_agenda_notes, delete_agenda_note
...
def create_calendar_tab(app):
    ...
    ctk.CTkLabel(sidebar, text="Détails de la journée", font=app.font_large).grid(row=2, column=0, pady=(0, 5), padx=20, sticky="w")
    app.cal_selected_date_label = ctk.CTkLabel(sidebar, text="Sélectionnez un jour", font=app.font_title, text_color="#3498db")
    app.cal_selected_date_label.grid(row=3, column=0, pady=(0, 20), padx=20, sticky="w")

    # Bouton Ajouter Note
    ctk.CTkButton(sidebar, text="+ Ajouter Note / RDV", command=lambda: _open_add_note_dialog(app), font=app.font_button, fg_color="#546E7A").grid(row=4, column=0, padx=20, pady=(0, 10), sticky="ew")

    app.cal_details_scroll = ctk.CTkScrollableFrame(sidebar, fg_color="transparent", label_text="")
    app.cal_details_scroll.grid(row=5, column=0, sticky="nsew", padx=10, pady=10)
...
def _refresh_calendar_view(app):
    ...
    for r, week in enumerate(cal):
        for c, day in enumerate(week):
            ...
            day_color = "#3498db" if is_today else ("black", "white")
...
def _update_details_sidebar(app, date):
    ...
    if day_notes:
        ctk.CTkLabel(app.cal_details_scroll, text="AGENDA / NOTES", font=("Montserrat", 12, "bold"), text_color="#8e44ad").pack(anchor="w", pady=(15, 5))
        for note in day_notes:
            f = ctk.CTkFrame(app.cal_details_scroll, fg_color=("white", "gray25"))
            f.pack(fill="x", pady=2)
            
            # Poignée de déplacement (Drag Handle)
            handle = ctk.CTkLabel(f, text="::", font=("Arial", 16), text_color="gray", cursor="fleur")
            handle.pack(side="left", padx=(5, 0), pady=5, fill="y")
            ...
            time_str = f"{note.get('time')} - " if note.get('time') else ""
            ctk.CTkLabel(header, text=f"{time_str}{note.get('title')}", font=("Montserrat", 12, "bold")).pack(side="left")
            
            # Bouton supprimer
            ctk.CTkButton(header, text="✕", width=20, height=20, fg_color="transparent", text_color="red", 
                          command=lambda nid=note['id'], y=year: _delete_note(app, nid, y)).pack(side="right")
            
            if note.get('description'):
                ctk.CTkLabel(f, text=note.get('description'), font=("Montserrat", 11), text_color="gray").pack(anchor="w", padx=5, pady=(0, 5))

    if day_inv.empty and day_exp.empty and not day_notes:
        ctk.CTkLabel(app.cal_details_scroll, text="Aucune activité enregistrée.", text_color="gray").pack(pady=20)
