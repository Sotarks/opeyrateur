...
from opeyrateur_app.ui.tabs.dashboard import update_dashboard_kpis
...
class SettingsUI:
    ...
    def _generate_random_invoices(self, count_entry, window):
        ...
        from opeyrateur_app.core.data_manager import save_to_excel, get_yearly_invoice_count
    ...
    def _regenerate_all_invoice_pdfs(self):
        if not messagebox.askyesno("Confirmation", "Cette action va SUPPRIMER TOUS vos PDF actuels dans le dossier factures, et les recréer de zéro à partir de votre base de données.\n\nIdéal pour restaurer vos fichiers après un changement d'ordinateur !\n\nContinuer ?"):
            return

        from opeyrateur_app.core import config
        try:
            if os.path.exists(config.FACTURES_DIR):
                shutil.rmtree(config.FACTURES_DIR)
            os.makedirs(config.FACTURES_DIR, exist_ok=True)
        except Exception as e:
            messagebox.showerror("Erreur", f"Impossible de nettoyer le dossier factures :\n{e}")
            return
